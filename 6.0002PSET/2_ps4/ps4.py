...
class SimpleBacteria(object):
    ...
    def is_killed(self):
		...
		"""
        # Checking if the bakteria is killed or not
    ...
    def reproduce(self, pop_density):
		...
		"""
        # Check if the bateria will reproduce
...
class Patient(object):
    ...
    def __init__(self, bacteria, max_pop):
		...
		this patient
		"""
        # Initiliazing the valus for the patient
    ...
    def update(self):
		...
		"""
        survived_bacteria = []
        # Creates a new list of survived bacteria
        for bact in self.bacteria:
            if not bact.is_killed():
                survived_bacteria.append(bact)
        # Calculating the new density
        pop_density = len(survived_bacteria) / self.max_pop
        new_bact = survived_bacteria[:]
        for bacteria in survived_bacteria:
            try:
                child = bacteria.reproduce(pop_density)
            except:
                child = None
            if child and len(new_bact) <= self.max_pop:
                new_bact.append(child)
        self.bacteria = new_bact
        return len(self.bacteria)
...
def calc_pop_avg(populations, t):
    """
	...
	Returns:
	float: The average bacteria population size at time step t
	"""
    sum_pop = 0
    # Sum over all of the populations at time t and then calculate the average
    for i in range(len(populations)):
        sum_pop += populations[i][t]
...
def calc_pop_std(populations, t):
	...
	* compute average squared distance of the data points from the average
	...
	"""
    # To calculate SD we need to find ((x-mu)^2/N)^.5
...
def plot_simulation_without_antibiotic(populations):
    ...
    Axis labels and plot title should be present. Use the code at the bottom of this
...
class ResistantBacteria(SimpleBacteria):
    ...
    def reproduce(self, pop_density):
		...
		"""
        if random.random() < self.birth_prob * (1 - pop_density):
            resistance = random.random() < self.mut_prob
            new_bacterial = ResistantBacteria(self.birth_prob, self.death_prob, self.mut_prob)
            return new_bacterial
        raise NoChildException()
...
class TreatedPatient(Patient):
    ...
    def __init__(self, bacteria, max_pop):
		...
		method.
		"""
        Patient.__init__(bacteria, max_pop)
...
def simulation_with_antibiotic(num_bacteria,
                               ...
                               num_trials):
    """
	Runs simulations for problem 4.
