...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if capacity > self.tiles[(m,n)] and capacity >= 0:
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_in_room(new_pos):
            m,n = new_pos.get_x(), new_pos.get_y()
        ...
        # If the position is not valid it will change the robot direction randomly
            ...
            self.set_robot_direction(random.randint(0,365))
...
class SuperRobot(Robot):
    ...
    2. If it can move, it moves and cleans the tile it moves to. Then, it tries to move a second time. 
        ...
        a. If it hits the wall, with probability p, it dirties the tile it’s on, and turns to a random direction. 
           Then it stops. 
