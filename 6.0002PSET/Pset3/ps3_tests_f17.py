...
# Fall 2017
# Changes from Spring 2017:
# - FaultyRobot has been refactored to RobotWithACat
#   - gets_faulty --> gets_cat_interference
# This should help with running MOSS and making sure that
# students aren't blatantly copying from old solutions
import sys
import threading
import traceback
import unittest
...
class ps3_P1A(unittest.TestCase):
    ...
    def test_get_random_position(self):
        ...
        room = ps3.RectangularRoom(width, height, dirt_amount)
        sol_room = test.RectangularRoom(width, height, dirt_amount)
        freq_buckets = {}
        for i in range(50000):
            pos = room.get_random_position()
            # confirm from test that this is a valid position
            self.assertTrue(sol_room.is_position_in_room(pos)) 
            try:
    ...
    def test_get_num_tiles(self):
        ...
        for i in range(10):        
            ...
            sol_room_tiles = ps3.RectangularRoom(width, height, dirt_amount).get_num_tiles()
...
class ps3_P3(unittest.TestCase):
    """This  tests RectangularRoom and Standard robot in various ways"""
    ...
    def test_BoundaryConditions(self):
        "Test strict inequalities in random positions for the RectangularRoom and StandardRobot"
        for m in range(7000):
...
"""The SIMULATION_TIME_LIMIT, SimulationThread class and SimulationTester class
    ...
    and passes the tests for StandardRobot and RectangularRoom, there should not be any
    additional issue.
"""
...
class ps3_P5_Standard(SimulationTester):
    """test the simulation time cleaning the RectangularRoom with a StandardRobot"""
    def testSimulation1(self):
        "Test cleaning 100% of a 5x5 room"
        try:
            self.run_simulation(((142, 174),), (1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.StandardRobot))
        except:
...
class ps3_P5_WithACat(SimulationTester):
    """test the simulation time cleaning the RectangularRoom with a RobotWithACat"""
    def testSimulation1(self):
    ...
    def testSimulation2(self):
        ...
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1, 0.75, 100, ps3.RobotWithACat)
        self.assertTrue(220 <= x <=247, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation3(self):
        "Test cleaning 90% of a 10x10 room with RobotWithACat"
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1, 0.9, 100, ps3.RobotWithACat)
        self.assertTrue(358 <= x <=395, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation4(self):
    ...
    def testSimulation5(self):
        ...
        x = ps3.run_simulation(4, 1.0, 3, 10, 10, 5, 0.75, 100, ps3.RobotWithACat)
    ...
    def testSimulation6(self):
        ...
        x = ps3.run_simulation(5, 1.0, 3, 10, 10, 10, 0.9, 100, ps3.RobotWithACat)
        self.assertTrue(206 <= x <=215, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
...
if __name__ == "__main__":
    ...
    suite.addTest(unittest.makeSuite(ps3_P5_WithACat))
